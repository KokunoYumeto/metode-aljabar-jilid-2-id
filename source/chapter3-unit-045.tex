...
$n<0$ definisikan $I_i^n:=0$. Ini mendefinisikan bikompleks
...
Argumen tersebut sesungguhnya memberikan teknik untuk membentuk resolusi bagi
kedua variabel sekaligus, lalu ``menurunkan'' $F$ secara seimbang melalui kompleks
...
$\Hom$ eksak kiri terhadap kedua variabel. Untuk setiap $n\in\Z$ gunakan
definisi berikut.
...
\index{funktor Ext@funktor $\Ext$ ($\Ext$ functor)}
...
secara alami dapat dipandang sebagai objek dalam kategori $\Bbbk\dcate{Mod}$.
...
Topik berikutnya adalah funktor $\Tor$. Misalkan $R$ suatu gelanggang. Kategori
...
\index{funktor Tor@funktor $\Tor$ ($\Tor$ functor)}
...
Seperti pada kasus $\Ext$, diperoleh karakterisasi berikut.
...
\emph{resolusi datar}\index{resolusi datar@resolusi datar (flat resolution)}
...
Misalkan $A$ dan $B$ gelanggang; misalkan $X$ merupakan bimodul $(A,R)$ dan
...
$\Tor_n^R(\cdot,S)$ (atau $\Tor_n^R(S,\cdot)$) secara alami menjadi funktor
...
Untuk setiap $M$ yang merupakan bimodul $(R,R)$, gunakan definisi awal homologi Hochschild
...
Jika $\Bbbk$ merupakan medan, kita dapat membaca $\HHm_n(M)$ dan
...
$C_p\otimes D_q$ sama dengan $d^C_p\otimes\identity+(-1)^p\identity\otimes
	d^D_q$. Ini memberikan homomorfisme kanonik
\[
...
turunan lain. Struktur tersebut lebih tepat dirumuskan dalam bahasa
