% Terjemahan Bahasa Indonesia dari chapter2.tex baris 1245--1563.
...
$(FX^\bullet,Fd^\bullet)$ juga demikian; yang perlu diperiksa ialah
keeksakannya.
...
\begin{bukti}
Dengan dualitas, cukup membahas (L1)--(L3).

(L1) $\implies$ (L2). Barisan eksak berbentuk
$0\to\bullet\to\bullet\to\bullet$ berpadanan tepat dengan diagram kernel suatu morfisme;
...
\begin{bukti}
(E1) $\implies$ (E2). Ambil barisan eksak $(X^\bullet,d^\bullet)$ di
$\mathcal{A}$ dan uraikan kompleks tersebut menjadi barisan eksak pendek
...
$d^n=\iota^{n+1}\mathbf{d}^n$; indeks $n$ boleh dipilih sebarang, tetapi
...
Sebagai contoh, setiap ekuivalensi antara kategori abelian tentu merupakan
funktor eksak.
...
Misalkan $\mathcal{A}$ merupakan kategori abelian, $I$ sebarang kategori, dan
anggap bahwa setiap funktor $\alpha:I\to\mathcal{A}$ memiliki $\varinjlim$ (atau
$\varprojlim$). Berdasarkan Proposisi~\ref{prop:functor-cat-Abel}, bekali
...
$\Delta(L)$ ekuivalen dengan memberikan keluarga morfisme yang kompatibel
...
\end{tikzcd}\]
Di sini funktor pelupa ${}_{R\to S}\mathcal{F}$ tidak lain adalah operasi
memandang suatu modul-$S$ sebagai modul-$R$ melalui $f$. Pembahasan funktor
...
Keeksakan ini juga dapat diverifikasi langsung secara aljabar. Sebagai contoh,
...
(yaitu $d^{n+1}d^n=0$), atau barisan eksak (yaitu
...
${}_{R\to S}\mathcal{F}$ mempertahankan semua limit; hal ini dapat diperiksa langsung
...
\begin{bukti}
Cukup ditinjau kompleks tiga suku
...
(iii) $\implies$ (i): Kondisi ini langsung menyiratkan bahwa $F$ eksak.
...
Lokalisasi (lihat \S\ref{sec:cat-localization}) memberikan contoh penting
funktor eksak.
...
Contoh lain yang sangat penting adalah funktor $\Hom$. Jika $T$ merupakan objek
...
hanya sedikit memengaruhi pembahasan berikutnya sehingga tidak akan diuraikan tersendiri.
...
kernel dalam teori grup, pernyataan ini langsung mengikuti dari sifat universal
$\Ker$.
\end{bukti}
...
injektif. Gunakan karakterisasi di atas pada diagram komutatif
...
\[\begin{tikzcd}
	...
	& X' &
\end{tikzcd}\]
Relasi $rf=\identity_{X'}$ kemudian memenuhi kondisi dalam
Proposisi~\ref{prop:split-ses}.
\end{bukti}
...
$\mathcal{A}$. Jika koproduk $\coprod_{i\in I}X_i$ (atau produk
	$\prod_{i\in I}X_i$) ada dalam $\mathcal{A}$, maka koproduk tersebut projektif
...
\mathcal{A}\to\cate{Ab}.
\]
Cukup digunakan pengamatan elementer berikut: untuk keluarga
...
pelupa $\mathcal{F}:R\dcate{Mod}\to\cate{Ab}$ melalui isomorfisme yang memetakan
homomorfisme $\varphi:R\to X$ ke $\varphi(1)\in X$. Maka $\Hom(R,\cdot)$ eksak.
...
Berikut adalah satu karakterisasi sederhana.
...
$S\twoheadrightarrow\Image(h)$ merupakan epimorfisme, komposisi
$S\twoheadrightarrow\Image(h)\xrightarrow{\delta'}X$
...
Pernyataan tentang funktor eksak setia merupakan penerapan langsung dari
Proposisi~\ref{prop:faithful-exact}.
\end{bukti}

% segment-id: o014.li2.chapter2.exact-functors-injective-projective-objects.p017
Untuk menjalankan aljabar homologis pada kategori abelian, kita sering memerlukan
...
projektif ialah menggunakan funktor adjoin bagi funktor eksak; berikut rinciannya.
...
tersebut tepat berupa modul-$\Z$ yang dapat dibagi. Ini merupakan cara standar dalam
...
setiap modul-$R$ $X$, ambil himpunan pembangkit $A\subset X$;
...
untuk $X\in\Obj(\mathcal{A})$; definisi pada tingkat morfisme sudah jelas. Kita akan
...
Karena $\varinjlim$ dan $\varprojlim$ di $\mathcal{A}^{\mathbf{2}}$ dibangun
objek demi objek, funktor $\mathrm{ev}_0,\mathrm{ev}_1$ dan $H,L_0,L_1$ memang eksak.
Sisa (i) dan (ii) mengikuti dari hubungan adjoin yang dipenuhi
...
Untuk (iii), bahas terlebih dahulu kasus objek injektif. Ambil objek
...
argumennya tidak menimbulkan kesulitan baru yang mendasar. Latihan bab ini akan
menguraikannya.
