...
Pertama-tama, kita membahas cara merepresentasikan morfisme di antara jumlah
langsung dengan matriks. Argumen yang terkait sepenuhnya sama dengan kasus
...
Masih tinjau dekomposisi jumlah langsung
...
$X=\bigoplus_{i=1}^n X_i$. Jika selanjutnya $\mathcal{A}$ diasumsikan sebagai
kategori abelian, maka suatu keluarga idempoten
$e_1,\ldots,e_n\in\End(X)$ yang memenuhi (E1)---(E3), sebaliknya, menentukan
subobjek-subobjek dari $X$
...
\emph{kategori pseudoabelian}. Pembahasan lebih lanjut akan diberikan dalam
bagian latihan bab ini.
...
$g:X\to X''$, morfisme $s$ disebut suatu \emph{penampang} dari $g$ jika
$gs=\identity_{X''}$, sedangkan $r$ disebut suatu \emph{retraksi} dari $f$
...
Perhatikan pula bahwa keseluruhan pernyataan (i)---(v) dalam proposisi tersebut
bersifat swa-dual.
...
objek tak terurai. Persoalannya terbagi menjadi masalah keberadaan dan
keunikan. Untuk kasus modul, inilah isi Teorema Krull--Remak--Schmidt; lihat
...
\begin{definisi}\label{def:bichain}
	...
	ganda} di dalamnya berarti data $(X_n,\alpha_n,\beta_n)_{n=0}^\infty$,
	...
	\end{tikzcd}$
	merupakan morfisme di antaranya, dengan $\alpha_n$ epimorfisme dan $\beta_n$
...
\begin{bukti}
	...
	$\alpha_{2n-1}\cdots\alpha_n:X_n\to X_{2n}$ bersifat invertibel. Nyatakan
	inversnya dengan
...
\begin{bukti}
	Pokok persoalannya adalah membuktikan bahwa $X$ yang memenuhi syarat rantai
	...
	pernyataan (i) tidak berlaku untuk $X$, maka pasti terdapat subobjek tak nol
	$X_1,Y_1$ sedemikian sehingga $X=X_1\oplus Y_1$ dan pernyataan (i) tidak
	berlaku untuk $X_1$. Melanjutkan operasi ini pada $X_1$ memberikan
	...
	$X_n=X_{n+1}\oplus Y_{n+1}$. Morfisme-morfisme ini tidak pernah merupakan
...
Dalam penerapan, hal pentingnya adalah mengetahui kapan syarat rantai ganda
...
\begin{bukti}
	Diberikan rantai ganda $(X_n,\alpha_n,\beta_n)_{n=0}^\infty$ dengan
	$X_0=X$. Maka
	\[
	...
	$\beta_n$ merupakan monomorfisme, untuk $n\gg 0$ syarat Artinian memberikan
	diagram komutatif
