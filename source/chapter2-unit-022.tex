...
memenuhi $gf = 0$, yaitu
$X \xrightarrow{f} Y \xrightarrow{g} Z$. Ambil komposisi
$\Ker(g) \hookrightarrow Y \twoheadrightarrow \Coker(f)$ sehingga diperoleh
...
\begin{bukti}
	...
	Untuk memperoleh (ii), kita nyatakan bahwa terdapat monomorfisme unik
	...
	\end{tikzcd}
	\]
	Jika pernyataan ini diterima, dua faktorisasi epi--mono pada diagram dalam
	(ii), yaitu bingkai luarnya, dapat langsung dibaca. Isomorfisme pada jalur
	tengah kemudian diberikan secara unik oleh
	Proposisi~\ref{prop:epi-mono-uniqueness}.

	Persoalannya tinggal membangun diagram di atas. Karena kedua diagram saling
...
\begin{bukti}
	Menurut pola yang sudah dikenal dari
...
\begin{bukti}
	Ambil morfisme-morfisme $\iota$, $p$, dan seterusnya yang dimiliki
...
\begin{definisi}[Kompleks]\label{def:complex}
	...
	Misalkan $\mathcal{A}$ merupakan kategori aditif. Perhatikan suatu barisan
	...
	Barisan tersebut boleh hanya mempunyai berhingga banyak suku, menjulur tak
	...
	$X^\bullet$, atau $X$. Suku $X^n$ secara konvensional disebut suku dari
	kompleks $X$ yang berderajat $n$.
\end{definisi}

% segment-id: o014.li2.chapter2.first-look-at-complexes.p004
Bab ini terutama berfokus pada kasus kategori abelian, tempat kohomologi dan
keeksakan kompleks dapat dikaji.
...
\begin{catatan}[Homologi kompleks rantai]\label{rem:cochain-vs-chain}
	...
	semua definisi sebelumnya berlaku tanpa perubahan. Karena asal-usulnya dalam
	topologi, versi dengan penomoran menaik $(X^\bullet, d^\bullet)$ lazim
	...
	Untuk kategori aditif $\mathcal{A}$, salah satu cara yang seluruhnya
	dilakukan di dalam kategori itu sendiri ialah mencerminkan indeks dengan
	menetapkan
...
Keeksakan suatu kompleks dapat diperiksa sepotong demi sepotong. Selanjutnya,
...
\begin{description}
	...
	$X' \twoheadrightarrow \Image(f) \hookrightarrow \Ker(g)$. Kita nyatakan
...
Selain itu, andaikan $f$ dapat dimasukkan ke dalam suatu barisan eksak
berbentuk
