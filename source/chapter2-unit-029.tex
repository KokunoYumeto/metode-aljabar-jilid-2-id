% Terjemahan Bahasa Indonesia dari chapter2.tex baris 1564--1754.
...
tersebut merupakan kategori aditif atau kategori abelian.
...
% segment-id: o014.li2.chapter2.serre-subcategories-and-k0-groups.i003
	\item untuk setiap morfisme $f:X\to Y$, jika $X,Y\in\Obj(\mathcal{B})$, maka
		$\Ker(f)$ dan $\Coker(f)$ juga dapat dipilih sebagai objek $\mathcal{B}$.
\end{itemize}
\end{proposisi}
...
$0$ dan jumlah langsung hingga. Selain itu, penyama dapat dinyatakan sebagai
$\Ker$ (Catatan~\ref{rem:equalizer-Ker}), sedangkan kondisi tersebut
...
Akan ditunjukkan bahwa $S$ merupakan sistem multiplikatif dalam pengertian
Definisi~\ref{def:multiplicative-family}.

Jelas bahwa $S$ memuat semua morfisme identitas, sehingga (S1) berlaku.
Misalkan $f:X\to Y$ dan $g:Y\to Z$ merupakan anggota $S$. Berdasarkan
...
$S$ (hal ini dapat diperiksa langsung berdasarkan
Korolari~\ref{prop:fraction-zero}); hal ini menyiratkan
$X=\Coker[U\xrightarrow{0}X]\in\Obj(\mathcal{T})$. Sebaliknya, jika
$X\in\Obj(\mathcal{T})$, kita dapat mengambil $U=X$. Jadi
$\Ker(Q)=\mathcal{T}$.

Selanjutnya, kita verifikasi sifat universal tersebut. Jika $\Ker(F)\supset\mathcal{T}$,
...
Berikutnya, perlu diverifikasi bahwa $G$ eksak.
...
dikomposisikan dengan suatu isomorfisme yang berasal dari $S$, direpresentasikan
oleh morfisme dalam $\mathcal{A}$, maka pemeriksaan bahwa $G$ mempertahankan
kernel direduksi menjadi
...
dengan asumsi $\mathcal{A}$ merupakan kategori berdaya-baik
...
surjektif: pandang sebarang modul-$R[S^{-1}]$ $N$ sebagai modul-$R$; dapat
diperiksa langsung bahwa terdapat isomorfisme modul-$R[S^{-1}]$
$N\dotimes{R}R[S^{-1}]\simeq N$.

Jadi, tinggal membuktikan bahwa $G$ penuh dan setia. Ambil homomorfisme
modul-$R[S^{-1}]$
...
kedua proyeksinya.}
$M_0:=M_1\dtimes{M_2[S^{-1}]}M_2$, dengan pemetaan
...
aditif. Ambil modul-$\Z$ bebas $F$ dengan basis
...
dari $\mathcal{A}$. Selanjutnya, lambangkan citra
...
% segment-id: o014.li2.chapter2.serre-subcategories-and-k0-groups.i010
	\item dengan notasi dari \S\ref{sec:Abel-cat-subobjects}, untuk
...
\begin{bukti}
Ambil barisan eksak pendek $0\to0\to0\to0\to0$; ini memberikan (i).
...
Untuk (iv), tambahkan suku-suku nol dan perpanjang barisan eksak ke kedua arah
hingga tak terhingga. Tinjau barisan eksak pendek
...
\begin{catatan}
\index{masalah ukuran (size issue)}
Karena buku ini mengharuskan grup direalisasikan pada himpunan kecil, dan
mengingat Lema~\ref{prop:K0-prep} (ii), rumusan yang sepenuhnya teliti harus
mensyaratkan bahwa $\mathcal{A}$ memiliki suatu kerangka kecil
...
merupakan kompleks dalam kategori abelian $\mathcal{A}$ dengan hanya berhingga
...
Suku-suku pada ruas kanan saling meniadakan; dengan menata ulang diperoleh
$\sum_i(-1)^i[\Hm^i(X)]=\sum_i(-1)^i[X^i]$.
\end{bukti}

% segment-id: o014.li2.chapter2.serre-subcategories-and-k0-groups.ex003
\begin{contoh}
Ambil $\mathcal{A}$ sebagai kategori abelian yang terdiri atas ruang vektor
berdimensi hingga
...
menjadi trivial. Alasannya, untuk setiap ruang vektor-$D$ $V$, argumen
kardinalitas menjamin adanya ruang vektor-$D$ $W$ sedemikian sehingga
...
barisan eksak grup-grup aditif
...
Teorema~\ref{prop:Serre-quotient}, teorema tersebut bersama
\S\ref{sec:cat-localization} memberikan:
...
% segment-id: o014.li2.chapter2.serre-subcategories-and-k0-groups.i012
	\item selain itu, setelah mengganti $Y_j$, $Y'_j$, $Y''_j$,
	dan $f_j$, $g_j$ melalui isomorfisme di $\mathcal{B}$ yang berasal dari
	$S$, kita dapat memastikan adanya morfisme
	$u_j$, $v_j$ di $\mathcal{A}$ sedemikian sehingga
	$f_j=Q(u_j)$ dan $g_j=Q(v_j)$. Berdasarkan
	$Q(v_ju_j)=g_jf_j=0$, dan
	Korolari~\ref{prop:fraction-zero}, kita dapat melakukan penyesuaian lebih
	lanjut dengan anggota $S$ sehingga $v_ju_j=0$.
\end{compactitem}
...
Untuk barisan eksak seperti pada
Proposisi~\ref{prop:K0-exact-seq}, strategi yang selalu berhasil ialah
...
Materi ini termasuk dalam teori K, yang jangkauan penerapannya juga mencakup
kategori eksak, yang lebih umum daripada kategori abelian. Karena konstruksi
terkait didasarkan pada sudut pandang teori homotopi, buku ini tidak dapat
menguraikannya secara rinci; pembaca yang berminat dapat merujuk pada
