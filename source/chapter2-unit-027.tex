% Draf terjemahan Bahasa Indonesia dari chapter2.tex baris 1133--1244.
...
\begin{konvensi}\label{con:direct-sum}
    ...
    Berdasarkan konvensi, koproduk $\coprod_{i \in I} X_i$ dari suatu keluarga
objek $(X_i)_{i \in I}$ dalam kategori abelian (dengan asumsi koproduk itu ada)
ditulis dalam bentuk $\bigoplus_{i \in I} X_i$ dan disebut sebagai
...
$0 \to X' \to X \to X'' \to 0$, tepat salah satu dari $X'=0$ atau
$X'\rightiso X$ berlaku; secara ekuivalen, tepat salah satu dari
$X\rightiso X''$ atau $X''=0$ berlaku. Karena itu, kesederhanaan $X$ dalam
...
juga ekuivalen dengan $\End(X)$ yang merupakan gelanggang tak nol.

% segment-id: o014.li2.chapter2.simplicity-and-semisimplicity.lm001
\begin{lema}[Lema Schur]
    Tetapkan objek $X,Y$. Jika $X$ (atau $Y$) merupakan objek sederhana, maka
...
parsial $[0,X]$ mempunyai deret komposisi; deret terakhir ini juga disebut
...
$X=X_0\supsetneq\cdots\supsetneq X_r=0$. Hingga isomorfisme,
$X_i/X_{i+1}$ disebut \emph{faktor komposisi}. Faktor-faktor ini sederhana.
Multihimpunannya (dengan multiplicitas) dilambangkan dengan $\mathrm{JH}(X)$
...
Dengan mengubah-ubah $i\leftrightarrow j$, terlihat bahwa, hingga isomorfisme
...
multiplicitas) tepat sama dengan $\ell(X)$. Multihimpunan juga memiliki operasi
gabungan, yang berarti penjumlahan multiplicitas; operasi ini tetap
...
\begin{bukti}
    Teorema~\ref{prop:Abel-cat-isom-thm} (ii) menyisipkan himpunan terurut
parsial $\mathrm{Sub}_{X'}$ dan $\mathrm{Sub}_{X''}$ masing-masing sebagai
...
\begin{definisi}\label{def:semisimple}
    ...
    \end{itemize}
    Jika semua objek semisederhana (atau terbelah), maka $\mathcal{A}$ disebut
...
(dihitung dengan multiplicitas), dan $\ell(X)=n$.
\end{lema}
% segment-id: o014.li2.chapter2.simplicity-and-semisimplicity.pf004
\begin{bukti}
    Pertimbangkan deret komposisi
...
rantai turun ketat) dalam $\mathrm{Sub}_X$.
\end{catatan}
...
\begin{bukti}
    Diberikan barisan eksak pendek $0\to X'\to X\to X''\to0$, kondisi tersebut
menyiratkan $X\simeq X'\oplus X''$, sehingga $X''$ dapat disisipkan sebagai
subobjek dari $X$. Jadi persoalannya tereduksi untuk membuktikan bahwa setiap
...
rantai turun ketat $X\supsetneq Y_1\supsetneq Y_2\cdots$ berhenti setelah
...
Kita juga dapat menanyakan sebaliknya apakah objek semisederhana terbelah.
...
\begin{proposisi}\label{prop:ss-decomp}
    Untuk objek $X$ yang ditetapkan, pertimbangkan sifat-sifat berikut.
...
otomatis terbelah. Begitu pembaca menguasai definisi terkait, argumennya
serupa dengan kasus modul, sehingga hal ini diserahkan sebagai latihan dalam
bab ini.
\end{catatan}
