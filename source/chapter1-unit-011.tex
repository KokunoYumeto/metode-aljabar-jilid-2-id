...
Berdasarkan konvensi, biproduk yang bersesuaian dengan $n=0$ ditetapkan sebagai
objek nol.
...
\begin{proposisi}\label{prop:additive-cat-addition}
	...
	$\mathcal{C}$. Maka $f+g$ sama dengan komposisi sepanjang baris pertama
...
\begin{bukti}
	...
	\end{tikzcd}\]
	Akan tetapi, ini tidak lain daripada funktorialitas morfisme $\delta$ dalam
...
\begin{definisi}
	...
	\begin{itemize}
		...
		$f$.
	\end{itemize}
	Secara khusus, kedua komposisi $\Ker(f) \to X \to Y$ dan
	$X \to Y \to \Coker(f)$ sama dengan $0$ menurut definisi.
\end{definisi}
...
\begin{itemize}
	...
	$T \xrightarrow{g} X \xrightarrow{0} Y$ tentu merupakan
	$0$.
	...
	$hf=0$, komposisi $X \xrightarrow{0} Y \xrightarrow{h} T$ tentu merupakan
	$0$. Karena itu,
...
\begin{bukti}
	...
	$f(g_1-g_2)=0 \iff g_1-g_2=0$. Ini sama dengan mengatakan bahwa untuk
...
\begin{contoh}\label{eg:strict-morphism}
	...
	$\Coim(f) \to \Image(f)$ dalam \eqref{eqn:strict-morphism} tidak lain
	daripada isomorfisme kanonik dalam teori modul
