...
Konvensi notasi dalam buku ini sama dengan yang digunakan dalam Jilid I.
Walaupun agak panjang, konvensi tersebut dirangkum kembali di sini.
...
Panah $\rightiso$ menyatakan isomorfisme antara dua objek; kadang-kadang
isomorfisme itu ditulis tanpa arah sebagai $\simeq$. Notasi
$\xrightarrow{1:1}$ atau
$\stackrel{1:1}{\longleftrightarrow}$ menyatakan bijeksi antara dua himpunan,
...
Dalam diagram ini, $f$, $g$, dan $h$ dapat berupa pemetaan, homomorfisme,
atau morfisme dalam suatu kategori umum. Diagram komutatif yang lebih rumit
dipahami dengan cara serupa.
...
dalam $A$---yakni kardinalitas atau kardinalnya---ditulis $|A|$.
...
Jika sifat antisimetris tidak disyaratkan, diperoleh \emph{himpunan
...
\begin{itemize}
	...
	dengan mengganti $\geq$ oleh $\leq$. Pada umumnya, unsur seperti ini tidak
	tunggal.
	% segment-id: o014.li2.prelude.conventions.i002
	\item Misalkan $S$ adalah himpunan bagian dari $P$. Jika $b \in P$ memenuhi
	$\forall x \in S, \; x \leq b$, maka $b$ disebut batas atas $S$ dalam $P$.
...
Jika setiap pasangan unsur dalam himpunan terurut parsial $P$ dapat dibandingkan,
maka $P$ disebut himpunan terurut total. Jika $P$ tak kosong dan setiap
himpunan bagian tak kosongnya mempunyai unsur terkecil, maka $P$
...
\begin{itemize}
	...
	ordinal mana pun yang lebih kecil darinya, maka ordinal itu disebut
	\emph{kardinal};
...
Himpunan morfisme antara dua objek ditulis
...
\begin{itemize}
	...
	komponen. Definisi ini langsung meluas kepada produk sembarang keluarga
...
disebut \emph{kategori diskret}. Kategori diskret berkorespondensi satu-satu
...
kelas, sedangkan himpunan $\Hom$ antara setiap dua objek merupakan himpunan.
...
kategori semua grup (atau grup abelian) yang direalisasikan pada
...
Kategori besar sukar dihindari dalam banyak konstruksi, khususnya dalam teori yang
berkaitan dengan kategori funktor atau lokalisasi kategoris. Setiap kali kategori besar
mungkin muncul dan benar-benar menimbulkan konsekuensi substantif, hal itu akan
...
setia). Jika setiap objek dalam $\mathcal{D}$ isomorfik dengan suatu $FX$, maka
$F$ disebut surjektif secara esensial.
...
Dalam sebagian pustaka, suatu morfisme $\varphi: F \to G$ antara dua funktor
...
akan segera ditinjau kembali merupakan contoh dasar.
...
``kategori besar''. Sebagai kasus khusus dari kategori funktor, untuk
...
\begin{compactitem}
	...
	menentukan suatu objek dalam $\mathcal{C}$, sehingga
	...
	$\mathcal{C}$, sedangkan morfisme-morfismenya ialah persegi komutatif
...
$G^{\opp}$. Jika $H$ adalah subgrup dari $G$, indeks $H$ dalam $G$ ditulis $(G:H)$;
...
yang dapat dibalik membentuk grup perkalian $R^\times$. Karakteristik medan atau
...
Misalkan $r$ adalah unsur tak nol dari gelanggang komutatif $R$. Jika homomorfisme grup
...
menyatakan gelanggang bilangan bulat, medan bilangan rasional, medan
bilangan real, dan medan bilangan kompleks. Jika $q$ merupakan pangkat suatu
bilangan prima, maka $\F_q$ menyatakan medan hingga dengan $q$ unsur. Jika
$F \hookrightarrow E$ adalah pembenaman medan, perluasan medan terkait,
atau medan perluasannya, ditulis $E|F$ dan derajatnya ditulis $[E:F]$.
...
Notasi untuk kategori-kategori umum yang dibentuk oleh beberapa struktur
aljabar dasar adalah sebagai berikut.
...
\begin{center}\small \begin{tabular}{|c|c|}\hline
	...
	Modul-$R$ kiri atau kanan & $R\dcate{Mod}$ atau $\cated{Mod}R$ \\
	Ruang vektor-$\Bbbk$ & $\cate{Vect}(\Bbbk)$ \\
	Bimodul $(R,S)$ & $(R,S)\dcate{Mod}$ \\
	Aljabar-$\Bbbk$ & $\Bbbk\dcate{Alg}$ \\
	Aljabar-$\Bbbk$ komutatif & $\Bbbk\dcate{CAlg}$
	\\ \hline
...
maka ketika membahas bimodul $(R,S)$, struktur bimodul secara baku dianggap
berasal dari struktur modul-$R \dotimes{\Bbbk} S^{\opp}$ kiri. Dengan kata lain,
aksi kiri dan kanan oleh $\Bbbk$ disyaratkan berimpit.

% segment-id: o014.li2.prelude.conventions.p040
Grup homomorfisme antara dua modul-$R$ kiri ataupun dua modul-$R$ kanan
ditulis dalam bentuk $\Hom_R(X, Y)$. Untuk bimodul $(R,S)$ digunakan notasi serupa,
$\Hom_{(R, S)}(X, Y)$.

% segment-id: o014.li2.prelude.conventions.p041
Untuk gelanggang komutatif $R$ dan suatu himpunan multiplikatif $U$, lokalisasi
terkait ditulis $R[U^{-1}]$.

% segment-id: o014.li2.prelude.conventions.p042
Seperti pada kategori himpunan $\cate{Set}$, grup, gelanggang, modul, dan struktur
lain di sini secara baku dipahami sebagai struktur pada himpunan-$\mathcal{U}$, dengan
...
Pasangan tersebut juga dilengkapi dengan keluarga bijeksi kanonik
$\varphi_{X, Y}: \Hom_{\mathcal{C}'}\left(FX, Y \right) \rightiso
\Hom_{\mathcal{C}}\left(X, GY \right)$, yakni isomorfisme antarfunktor berikut:
...
\emph{kounit} $\varepsilon: FG \to \identity_{\mathcal{C}'}$. Data
$(F, G, \eta, \varepsilon)$ membentuk pasangan adjoin jika dan hanya jika
morfisme-morfisme tersebut memenuhi
...
$\alpha: I \to \mathcal{C}$ dapat diidentifikasi dengan $\varprojlim$ dari
$\alpha^{\opp}: I^{\opp} \to \mathcal{C}^{\opp}$.
Oleh karena itu, dalam buku ini funktor yang terkait dengan $\varprojlim$
...
Untuk menetapkan notasi, pilih kategori $\mathcal{C}$ dan
tinjau kembali beberapa kasus khusus limit yang lazim. Definisi terperinci
dapat ditemukan dalam \cite[\S~2.7]{Li1} atau buku ajar lain. Dalam
sifat-sifat universal berikut, $T$ menyatakan sembarang objek dalam
$\mathcal{C}$, sedangkan $I$ menyatakan sembarang himpunan.
...
Penyama dan kopenyama, serta produk dan koproduk, masing-masing membentuk
pasangan dual.
...
Produk serat yang melibatkan berhingga banyak objek
$X_1 \dtimes{Z} X_2 \dtimes{Z} \cdots$, maupun koproduk serat
$X_1 \dsqcup{Z} X_2 \dsqcup{Z} \cdots$, dapat ditangani dengan mengulang kasus dua objek.
...
Kedua diagram tersebut masing-masing disebut diagram \emph{tarik balik} dan
diagram \emph{dorong keluar}. Morfisme $X \dtimes{Z} Y \to Y$ disebut tarik balik dari
...
Dalam notasi buku ini, bagian tengah persegi pada diagram tarik balik (atau dorong
