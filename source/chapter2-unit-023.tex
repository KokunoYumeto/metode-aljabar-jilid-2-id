...
\begin{bukti}
	Pertama, tinjau arah ``hanya jika''. Morfisme $f$ dan $h$ sama-sama
	...
	yang diberikan oleh $gf=0$. Akan ditunjukkan bahwa diagram tersebut komutatif.
...
Sekarang kita membahas Lema Ular, yang sangat diperlukan dalam penerapan.
Tinjau diagram
...
Dengan demikian, \eqref{eqn:snake} merupakan diagram komutatif; setiap
kolom dan kedua baris tengahnya eksak. Berikut akan dijelaskan cara
...
Langkah pertama ialah menurunkan diagram komutatif berikut dari
\eqref{eqn:snake}; semua barisnya eksak:
...
Notasikan komposisi sepanjang jalur tengah dalam \eqref{eqn:snake-conn} sebagai
...
\begin{catatan}\label{rem:connecting-canonical}
	...
	komutatif yang bersesuaian. Semuanya mengikuti dari funktorialitas kernel,
...
\begin{itemize}
	...
	\[ \Ker \to \Ker'' \xrightarrow{\delta} \Coker' \hookrightarrow \Coker' \dsqcup{Y'} Y, \qquad \Ker \to X \dtimes{X''} \Ker'' \xrightarrow{\delta_0} \Coker' \dsqcup{Y'} Y \]
		sama. Namun, komposisi yang kedua juga sama dengan komposisi
...
\begin{bukti}
	...
	Bagian utama pembuktian ialah keeksakan barisan pertama.
	...
	Kita perlu membangun diagram komutatif
	...
	morfisme $\psi: S \to \Ker''$ memenuhi $\delta \psi = 0$. Kita perlu membangun
	...
	sama dengan $0$. Mulailah dengan diagram komutatif
	...
	\[\begin{tikzcd}
		...
		X \arrow[rr, "g"] & & X'' .
	\end{tikzcd}\]
	Untuk memperoleh diagram dalam
	...
	mereduksi menjadi diagram komutatif yang telah diketahui (lihat
	...
	\[\begin{tikzcd}
		...
		& X \arrow[r, "g"] & X'' .
	\end{tikzcd}\]
	Dengan demikian, semua syarat yang diperlukan terpenuhi.
\end{bukti}
...
\begin{bukti}
	...
	Andaikan morfisme $h: S \to X_3$ memenuhi $f_3 h = 0$; kita perlu
	membuktikan bahwa $h = 0$. Komposisi
	...
	\end{tikzcd}\]
	Caranya sebagai berikut. Karena $f_3 h = 0$, komposisi
	$S' \to X_2 \xrightarrow{f_2} Y_2 \to Y_3$
	...
	\[\begin{tikzcd}
		...
		Y_1 \arrow[r] & Y_2 & &
	\end{tikzcd}\]
	Akan ditunjukkan bahwa diagram ini komutatif. Satu-satunya persoalan ialah
	...
	\begin{tikzpicture}[scale=0.3]
		\draw[->] (0,1) -- (0,0) -- (1,0);
	\end{tikzpicture}
	di dalam persegi itu dengan $f_2$ di sebelah kanan. Karena kedua persegi
...
menemukan Lema Salamander yang bersifat menyeluruh; lema ini merangkum Lema Ular
