...
objek $X$ dalam $\mathcal{A}$, himpunan terurut parsial subobjek-subobjeknya
...
Hasil-hasil dalam bagian ini sudah dikenal baik untuk kasus
...
\begin{definisi}\label{def:intersection-sum}
	...
	\begin{itemize}
		...
		$X/X':=\Coker[X'\to X]$; secara alami, objek ini merupakan objek
		kuosien dari $X$. Setiap epimorfisme
...
ada. Peran definisi ini dijelaskan oleh fakta berikut.

% segment-id: o014.li2.chapter2.subobjects-and-isomorphism-theorems.pr001
\begin{proposisi}\label{prop:subobject-inf-sup}
	Diberikan suatu keluarga subobjek $(X_i)_{i\in I}$ dari $X$. Jika produk
...
objek kuosien $(\mathrm{Quot}_X,\twoheadleftarrow)$ tidak berbeda secara
hakiki. Hal ini berdasarkan bijeksi pembalik urutan yang jelas
...
\begin{definisi}\label{def:subobject-image}
	...
	yang pertama merupakan prabayang $Y'$ dan subobjek dari $X$, sedangkan
	yang kedua merupakan citra $X'$ dan subobjek dari $Y$. Konstruksi ini
...
\begin{lema}\label{prop:image-sum-preimage-intersection}
	...
	\begin{enumerate}[(i)]
		...
		\item Diberikan keluarga elemen $(X'_i)_{i\in I}$ dari
...
\begin{bukti}
	...
	menunjukkan bahwa diagram seperti itu berkorespondensi satu-satu dengan
...
sebagai persamaan $A\oplus B=X$. Bertolak dari fakta ini, kita dapat secara
rekursif menggunakan bahasa teori kisi untuk mencirikan apakah $X$ dapat
...
\begin{bukti}
	...
	\end{tikzcd}\]
	Panah putus-putus berasal dari funktorialitas kokernel
	...
	isomorfisme, sebagaimana ingin dibuktikan.
\end{bukti}
...
\begin{bukti}
	...
	$f$ dapat dilakukan secara bertahap, persoalan tereduksi pada dua kasus: $f$
	...
	$\Coker(g')=\Coker(g'')$. Dengan demikian, persoalan tereduksi pada kasus
...
\begin{bukti}
	...
	pada pernyataan berikut: untuk setiap $A,B,C\in[Y_1,Y_2]$,
