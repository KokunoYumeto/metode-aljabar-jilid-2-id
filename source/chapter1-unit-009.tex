...
epimorfisme. Misalkan $\mathcal{C}$ sebuah kategori.
...
Langsung terlihat bahwa $f: X \to Y$ merupakan monomorfisme jika dan
...
\begin{contoh}
	...
	$p: Y \to \Coker(f,g)$ merupakan epimorfisme. Pengamatan sederhana lain
...
\begin{proposisi}\label{prop:epi-mono-isomorphism}
	Tinjau morfisme-morfisme $X \xrightarrow{a} Y \xrightarrow{b} Z$. Andaikan
...
\begin{bukti}
	Salah satu arah sudah jelas. Selanjutnya, andaikan $ba$ merupakan
	isomorfisme dan $b$ merupakan monomorfisme. Definisikan
...
sebagai pemetaan. Dalam contoh-contoh tersebut, kita juga dapat membicarakan
himpunan bagian, subgrup, dan sebagainya. Konsep itu dapat diperluas ke
...
Keunikan memberikan $g_2 g_1 = \identity_{S_1}$, dan demikian pula
...
\begin{definisi}\label{def:sub-quot-obj}
	...
	Misalkan $\mathcal{C}$ sebuah kategori dan $X$ sebuah objek di dalamnya. Kelas
...
\begin{bukti}
	Berdasarkan dualitas, cukup dibahas kasus monomorfisme. Misalkan
	$p_2: X \dtimes{Z} Y \to Y$ adalah morfisme kanonik yang diberikan oleh
	produk serat. Untuk setiap objek $T$, cukup dibuktikan bahwa
...
\begin{definisi}\label{def:subquotient}
	...
	Misalkan $\mathcal{C}$ kategori sembarang dan $X,Y$ objek di dalamnya. Jika
	$Y$ dapat direalisasikan sebagai objek kuosien dari suatu
...
bahwa, untuk kelas kategori tertentu, kedua cara tersebut memberikan pengertian
...
\begin{bukti}
	...
	$\mathcal{C}$. Definisikan $W := X \dsqcup{Z} Y$. Morfisme $X \to W$ adalah
...
Berdasarkan kesetaraan ``objek kuosien dari subobjek = subobjek dari objek
kuosien'' yang baru dibuktikan, subkuosien dari subkuosien tetap merupakan
subkuosien.
