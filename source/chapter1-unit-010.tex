...
Tetapkan sebuah kategori $\mathcal{C}$. Untuk setiap morfisme $f: X \to Y$,
...
\begin{definisi}[Citra dan Kocitra]\label{def:Im-Coim}
	...
	Misalkan $f: X \to Y$ adalah morfisme dalam kategori $\mathcal{C}$.
...
Lema berikut tidak hanya berguna, tetapi juga menjelaskan dengan baik hakikat
citra dan kocitra.
...
Dengan menerapkan Lema~\ref{prop:Im-Coim-fac} berturut-turut kepada
...
ketat, dan citra serta kocitra keduanya dapat diidentifikasi secara kanonik
...
\begin{proposisi}\label{prop:Images-f-gf}
	Diberikan morfisme-morfisme $X \xrightarrow{f} Y \xrightarrow{g} Z$,
...
\begin{bukti}
	Untuk versi $\alpha$, keunikan berasal dari hukum kanselasi kanan bagi
...
\begin{lema}\label{prop:epi-mono-morphism}
	Tinjau diagram berikut, yang bagian berpanah utuhnya komutatif,
...
\begin{bukti}
	...
	Argumen di atas membuktikan masing-masing separuh dari (i) dan (ii). Untuk
...
\begin{definisi}
	Tetapkan $f: X \to Y$. Bentuklah kategori $\cate{epi.mono}(f)$ yang
...
monomorfisme. Dengan kata lain, $\cate{epi.mono}(f)$ berasal dari sebuah
himpunan praterurut. Dalam bagian ini, kita hanya meninjau satu kasus khusus
...
\begin{bukti}
	...
	keunikannya, Proposisi~\ref{prop:strict-isomorphism} dan pembahasan
	sebelumnya menunjukkan bahwa setiap morfisme dalam $\cate{epi.mono}(f)$
	merupakan isomorfisme. Jadi, cukuplah memberikan sebuah morfisme
